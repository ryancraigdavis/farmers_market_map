...
function addServiceList(){
    ...
    req.open('POST', 'http://66.42.74.237:1359/api/services/add_service_list', true);
...
function deleteServicesList(service){
    ...
    req.open('DELETE', 'http://66.42.74.237:1359/api/services/delete_service_list', true);
    req.setRequestHeader('Content-Type', 'application/json');
    req.addEventListener('load',function(){
      	if(req.status >= 200 && req.status < 400){
	    	data = req.responseText;
...
function updateServicesList(service){
  console.log(service);
    ...
    req.open('PUT', 'http://66.42.74.237:1359/api/services/update_service_list', true);
    req.setRequestHeader('Content-Type', 'application/json');
    req.addEventListener('load',function(){
      	if(req.status >= 200 && req.status < 400){
	    	data = req.responseText;
...
function loadServiceList(markets){
  ...
  var req = new XMLHttpRequest();
    ...
    req.open('POST', 'http://66.42.74.237:1359/api/services/get_service_lists', true);
    req.setRequestHeader('Content-Type', 'application/json');
    req.addEventListener('load',function(){
        if(req.status >= 200 && req.status < 400){
        data = JSON.parse(req.responseText);
